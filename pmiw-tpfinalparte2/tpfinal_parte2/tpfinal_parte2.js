...
function preload() {
  musica = loadSound("data/musica.mp3");   // ← TU ARCHIVO
  sonido = loadSound("data/sonidos.mp3");  // ← TU ARCHIVO
...
function draw() {
  if (pantalla == "inicio") {
    ...
    if (xMinotauro > width - 40 || xMinotauro < 40) velocidadMinotauro *= -1;
...
function keyPressed() {
  if (pantalla == "juego") {
    if (keyCode === LEFT_ARROW) xTeseo -= 10;
      ...
      sonido.play(); // sonido del disparo
    }
  }
}
